...
if [ -z "${PEBBLE_PID}" ] && [ -z "${PEBBLE_CHALLTESTSRV_PID}" ]; then
  echo "neither pebble nor pebble-challtestsrv are running; do not need to stop.">&2
  exit 0
fi
...
if [ "${PEBBLE_ERROR}" == "true" ] || [ "${PEBBLE_CHALLTESTSRV_ERROR}" == "true" ]; then
  exit 1
fi
